...
void eigen_solve_submatrix(PulseMatrix& mat, PulseVector& invec, PulseVector& outvec, unsigned NP) {
  ...
  default:
    ;
    // TODO: can we do something better actually
    //printf("multifit weird state\n");
...
__device__
bool
PulseChiSqSNNLS::DoFit(const SampleVector &samples, const SampleMatrix &samplecov, const BXVector &bxs, const FullSampleVector &fullpulse, const FullSampleMatrix &fullpulsecov, const SampleGainVector &gains, const SampleGainVector &badSamples) {
  ...
  if (_bxs.rows()==1 && ecal::abs(static_cast<int>(_bxs.coeff(0)))<100) {
  ...
  double sigmaplus = ecal::abs(xplus100-x0)/sqrt(chisqplus100-chisq0);
  ...
  if ( (x0/sigmaplus) > 0.5 ) {
   ...
   _sampvec = samples - _ampvec.coeff(ipulseintime)*pulseintime;
    ...
    double sigmaminus = ecal::abs(xminus100-x0)/sqrt(chisqminus100-chisq0);
...
//          printf("PulseChiSqSNNLS::Minimize max iterations reached at iter\n");
...
bool PulseChiSqSNNLS::updateCov(const SampleMatrix &samplecov, const FullSampleMatrix &fullpulsecov) {
  ...
  for (unsigned int ipulse=0; ipulse<npulse; ++ipulse) {
    ...
    int bx = _bxs.coeff(ipulse);
    if (ecal::abs(bx)>=100) continue; //no contribution to covariance from pedestal or saturation/slew step correction
...
bool PulseChiSqSNNLS::NNLS() {
  ...
  const unsigned int npulse = _bxs.rows();
  const unsigned int nsamples = SampleVector::RowsAtCompileTime;
  ...
  while (true) {
    ...
    if (iter>0 || _nP==0) {
      ...
      if (iter>=500) {
          // TODO: Can we do better
          //printf("PulseChiSqSNNLS::NNLS() max iterations reached at iter %d\n", iter);
...
void PulseChiSqSNNLS::NNLSUnconstrainParameter(Index idxp) {
  ...
  Eigen::numext::swap(aTbvec.coeffRef(_nP),aTbvec.coeffRef(idxp));
  Eigen::numext::swap(_ampvec.coeffRef(_nP),_ampvec.coeffRef(idxp));
  Eigen::numext::swap(_bxs.coeffRef(_nP),_bxs.coeffRef(idxp));
  ++_nP;
}
...
void PulseChiSqSNNLS::NNLSConstrainParameter(Index minratioidx) {
  ...
  Eigen::numext::swap(aTbvec.coeffRef(_nP-1),aTbvec.coeffRef(minratioidx));
  Eigen::numext::swap(_ampvec.coeffRef(_nP-1),_ampvec.coeffRef(minratioidx));
  Eigen::numext::swap(_bxs.coeffRef(_nP-1),_bxs.coeffRef(minratioidx));
  --_nP;
