...
EcalLaserAPDPNRatiosRefGPU::Product const& EcalLaserAPDPNRatiosRefGPU::getProduct(cuda::stream_t<>& cudaStream) const {
    auto const& product = product_.dataForCurrentDeviceAsync(cudaStream,
        [this](EcalLaserAPDPNRatiosRefGPU::Product& product, cuda::stream_t<>& cudaStream) {
            // malloc
