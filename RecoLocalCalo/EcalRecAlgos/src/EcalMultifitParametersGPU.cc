...
EcalMultifitParametersGPU::EcalMultifitParametersGPU(edm::ParameterSet const& ps) {
    auto const& amplitudeFitParametersEB = ps.getParameter<std::vector<double>>(
        "EBamplitudeFitParameters");
    auto const& amplitudeFitParametersEE = ps.getParameter<std::vector<double>>(
    ...
    std::copy(amplitudeFitParametersEB.begin(), amplitudeFitParametersEB.end(),
        amplitudeFitParametersEB_.begin());
    std::copy(amplitudeFitParametersEE.begin(), amplitudeFitParametersEE.end(),
        amplitudeFitParametersEE_.begin());
    std::copy(timeFitParametersEB.begin(), timeFitParametersEB.end(),
        timeFitParametersEB_.begin());
    std::copy(timeFitParametersEE.begin(), timeFitParametersEE.end(),
        timeFitParametersEE_.begin());
}
...
EcalMultifitParametersGPU::Product const& EcalMultifitParametersGPU::getProduct(cudaStream_t cudaStream) const { 
    auto const& product = 
        product_.dataForCurrentDeviceAsync(cudaStream, [this](EcalMultifitParametersGPU::Product& product, cudaStream_t cudaStream) {
            // malloc
            cudaCheck(cudaMalloc((void**)&product.amplitudeFitParametersEB, this->amplitudeFitParametersEB_.size() * sizeof(double)));
            cudaCheck(cudaMalloc((void**)&product.amplitudeFitParametersEE, this->amplitudeFitParametersEE_.size() * sizeof(double)));
