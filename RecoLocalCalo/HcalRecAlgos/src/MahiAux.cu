...
namespace FitterFuncs{
  ...
  __device__ void MahiFunctor::assign(float const *pulse,
    ...
    for(int i=0; i<HcalConst::maxPSshapeBin; ++i) {
        for(int j=i; j<i+HcalConst::nsPerBX; ++j) {  
            //sum over HcalConst::nsPerBXns from point i
  ...
  __device__ void MahiFunctor::funcShape(double *ntmpbin, const double pulseTime, 
    ...
    double offset_start = i_start - HcalConst::iniTimeShift - pulseTime - slew; 
   
    //-199-2*pars[0]-2.*slew (for pars[0] > 98.5) or just -98.5-pars[0]-slew;
    // zeroing output binned pulse shape
    for (auto i=0; i<HcalConst::maxSamples; i++)
        ntmpbin[i] = 0.0;
  ...
  __device__ double MahiFunctor::EvalPulse(const double *pars, const unsigned nPars) {
   ...
   __device__ double MahiFunctor::singlePulseShapeFunc( const double *x ) {
      return EvalPulse(x,3);
   }
  
   __device__ double MahiFunctor::doublePulseShapeFunc( const double *x ) {
      return EvalPulse(x,5);
   }
  
   __device__ double MahiFunctor::triplePulseShapeFunc( const double *x ) {
