...
EcalUncalibRecHitProducerGPUNew::EcalUncalibRecHitProducerGPUNew(const edm::ParameterSet& ps)
{
...
EcalUncalibRecHitProducerGPUNew::~EcalUncalibRecHitProducerGPUNew()
{
        delete worker_;
}

void EcalUncalibRecHitProducerGPUNew::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
 ...
 
  {
    ...
    desc.ifValue(edm::ParameterDescription<std::string>("algo", "EcalUncalibRecHitWorkerMultiFitGPUNew", true), std::move(s));
  ...
  for (std::vector<edmplugin::PluginInfo>::const_iterator itInfos = infos.begin(); itInfos != infos.end(); itInfos++) {
    ...
    descriptions.add("ecal"+algoName+"UncalibRecHitGPUNew", desc);
//    descriptions.addDefault(desc);
  }
...
void
EcalUncalibRecHitProducerGPUNew::produce(edm::Event& evt, const edm::EventSetup& es) {

    DurationMeasurer<std::chrono::milliseconds> timer{std::string{"per event timer"}};
	...
	edm::LogInfo("EcalUncalibRecHitInfo") << "total # eeDigis: " << eeDigis->size() ;
        ...
        worker_->run(evt, *ebDigis, *eeDigis, *eb_rechits_soa, *ee_rechits_soa);
...
DEFINE_FWK_MODULE( EcalUncalibRecHitProducerGPUNew );
