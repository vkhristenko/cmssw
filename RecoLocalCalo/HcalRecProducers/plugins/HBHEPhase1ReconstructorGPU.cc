...
HBHEPhase1ReconstructorGPU::~HBHEPhase1ReconstructorGPU()
{
   ...
   // (e.g. close files, deallocate resources etc.)
    ...
    // gpu specific
    //
    cudaStreamDestroy(m_custream);
}

void HBHEPhase1ReconstructorGPU::test_gpu() {
    //
    // a gpu version of running the reconstruction
    //
//    auto first = coll.begin();
//    auto last = coll.end();
#ifdef GPU_REFACTOR_DEBUG
    std::cout << "gpu stuff" << std::endl;
#endif
    ...
    hcal::test::init_host_vector(h_a, N, [](int i){ return i;}); 
    hcal::test::init_host_vector(h_b, N, [](int i){ return i+1;}); 

    cudaMalloc((void**)&d_a, N*sizeof(int));
    cudaMalloc((void**)&d_b, N*sizeof(int));
    cudaMalloc((void**)&d_c, N*sizeof(int));

    cudaMemcpyAsync(d_a, h_a, N*sizeof(int), cudaMemcpyHostToDevice, m_custream);
    cudaMemcpyAsync(d_b, h_b, N*sizeof(int), cudaMemcpyHostToDevice, m_custream);

    hcal::test::vector_add(d_a, d_b, d_c, N, m_custream);

    cudaMemcpyAsync(h_c, d_c, N*sizeof(int), cudaMemcpyDeviceToHost, m_custream);
                                          ...
                                          HBHERecHitCollection* rechits)
{
         ...
         it != coll.end(); ++it)
    {
        ...
        LogDebug("Hcal") << "processing channel " << cell; 
        ...
        LogDebug("Hcal") << "getting a bunch of conditions";
        ...
        LogDebug ("Hcal") << "getting conditions for sipms";
        ...
        LogDebug("Hcal") << "getting adc2fc converter";
        ...
        LogDebug("Hcal") << "for each ts do some manip";
        ...
        LogDebug ("Hcal") << "set channel info";
        ...
        if (rechits && makeThisRechit)
        {
            ...
            if (recoParamsFromDB_)
                pptr = param_ts;
            LogDebug ("Hcal") << "perform reconstruction";
...
HBHEPhase1ReconstructorGPU::produce(edm::Event& e, const edm::EventSetup& eventSetup)
{
    ...
    LogDebug("Hcal") << "start hbhe reconstruction";
    ...
    {
        LogDebug("Hcal") << "getting negative filter";
    ...
    if (processQIE11_)
    {
        LogDebug("Hcal") << "processing qie11";
