...
//	ib.setCurrentFolder(rootFolder_+"ZDCTask");
	for (unsigned int i=0; i<_vDetIds.size(); i++)
	{
		...
		ib.setCurrentFolder(rootFolder_+"ZDCTask/NumDigis");
		_NumDigisvsLS = ib.bookProfile("NumDigis", "NumDigis",
			4000, 0.5, 4000.5, 0, 100);
		ib.setCurrentFolder(rootFolder_+"ZDCTask/NumTimeSamples");
		_NumTimeSamplesvsLS = ib.bookProfile("NumTimeSamples", "NumTimeSamples",
			4000, 0.5, 4000.5, 0, 20);
	}
...
void ZDCMonitorModule::analyze(const edm::Event& e, const edm::EventSetup& eventSetup)
{
	...
	*/
		...
		it!=zdc_digi->end(); ++it)
	{
		...
		_NumTimeSamplesvsLS->Fill(e.luminosityBlock(), it->size());
	}
	_NumDigisvsLS->Fill(e.luminosityBlock(), counter);
