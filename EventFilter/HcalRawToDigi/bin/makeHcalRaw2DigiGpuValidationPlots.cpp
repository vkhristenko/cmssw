...
QIE11DigiCollection filterQIE11(QIE11DigiCollection const& coll) {
    QIE11DigiCollection out; out.reserve(coll.size());
...
int main(int argc, char *argv[]) {
    ...
    rt->SetBranchAddress(
        ...
        &wgpuf01he);
    rt->SetBranchAddress(
        "hcalFlavor3hcalCUDAHostAllocatorAliashcalcommonVecStoragePolicyhcalDigiCollection_hcalCPUDigisProducer_f3HBDigis_RECO.",
        &wgpuf3hb);
    ...
    for (int ie=0; ie<nentries; ++ie) {
        ...
        if (ngpuf3hb != ncpuf3hb) {
            std::cerr << "*** mismatch in number of flavor 3 digis for event "
                      << ie
                      << std::endl
        ...
        
        {
            ...
            for (uint32_t ich=0; ich<ncpuf3hb; ich++) {
                ...
                auto const ptrdiff = iter2idgpu - idsgpu.begin();
                auto const nsamples_gpu = hcal::compute_nsamples<hcal::Flavor3>(
                    f3HBProduct.stride);
                ...
                for (uint32_t sample=0u; sample<nsamples_gpu; sample++) {
                    auto const cpuadc = cpudf[sample].adc();
                    auto const gpuadc = hcal::adc_for_sample<hcal::Flavor3>(
                        df_start, sample);
                    auto const cputdc = cpudf[sample].tdc();
                    auto const gputdc = hcal::tdc_for_sample<hcal::Flavor3>(
                        df_start, sample);
