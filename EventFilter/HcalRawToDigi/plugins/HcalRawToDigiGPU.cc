...
HcalRawToDigiGPU::HcalRawToDigiGPU(edm::ParameterSet const& conf):
  ...
  expectedOrbitMessageTime_(conf.getUntrackedParameter<int>("ExpectedOrbitMessageTime",-1))
{
  ...
  for (unsigned int i=0; i<fedUnpackList_.size(); i++) 
    ss << fedUnpackList_[i] << " ";
  edm::LogInfo("HCAL") << "HcalRawToDigiGPU will unpack FEDs ( " << ss.str() << ")";
...
void HcalRawToDigiGPU::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  edm::ParameterSetDescription desc;
  desc.addUntracked<int>("HcalFirstFEDGPU",int(FEDNumbering::MINHCALFEDID));
  desc.add<int>("firstSampleGPU",0);
  desc.add<int>("lastSampleGPU",9);
  desc.add<bool>("FilterDataQualityGPU",true);
  desc.addUntracked<std::vector<int>>("FEDsGPU", std::vector<int>());
  desc.addUntracked<bool>("UnpackZDCGPU",true);
  desc.addUntracked<bool>("UnpackCaliGPUb",true);
  desc.addUntracked<bool>("UnpackUMNioGPU",true);
  desc.addUntracked<bool>("UnpackTTPGPU",true);
  desc.addUntracked<bool>("silentGPU",true);
  desc.addUntracked<bool>("ComplainEmptyDataGPU",false);
  desc.addUntracked<int>("UnpackerModeGPU",0);
  desc.addUntracked<int>("ExpectedOrbitMessageTimeGPU",-1);
  desc.add<edm::InputTag>("InputLabelGPU",edm::InputTag("rawDataCollector"));
  desc.add<std::string>("ElectronicsMapGPU","");
  descriptions.add("hcalRawToDigiGPU",desc);
}


// Functions that gets called by framework every event
void HcalRawToDigiGPU::produce(edm::Event& e, const edm::EventSetup& es)
{
  // Step A: Get Inputs 
  ...
  if( cntHFdup ) edm::LogError("HcalRawToDigiGPU") << "Duplicated HF digis found for "<<cntHFdup<<" times"<<std::endl;
